...
public class InvoicePreviewDialog extends JDialog {    private Order order;
    ...
    public InvoicePreviewDialog(JFrame parent, int orderId) {
        ...
        products = loadProductsFromOrderProductsTable(orderId);
        
        if (products.isEmpty()) {
            System.out.println("WARNING: No products found for order " + orderId);
            // Create some dummy products to prevent empty invoice
            for (int i = 1; i <= 3; i++) {
                Product dummyProduct = new Product(
                    i, "Komponente", "Produkt " + i, "Hersteller " + i,
                    99.99 * i, "Verfügbar", 1, "Beispielprodukt " + i
                );
     ...
     */    private boolean createInvoiceForOrder(Order order) {
        ...
        try {
            // Calculate total price from order_products table
            double totalPrice = 0.0;
            List<Product> orderProducts = loadProductsFromOrderProductsTable(order.getOrderId());
            
            for (Product product : orderProducts) {
                totalPrice += product.getPrice() * product.getItemCount();
            }
            
            // If no products found, use a default price
            if (orderProducts.isEmpty()) {
                totalPrice = 299.99; // Default invoice amount
                System.out.println("No products found for invoice calculation, using default amount: " + totalPrice);
            }
