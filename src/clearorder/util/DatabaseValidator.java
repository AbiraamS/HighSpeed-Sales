...
public class DatabaseValidator {
    ...
    public static void main(String[] args) {
        ...
        validatePCConfigurationsTable(); // Add PC configurations validation
    }
    ...
    }    public static void validatePCConfigurationsTable() {
        System.out.println("Validating pc_configurations table...");
        
        try (Connection conn = DatabaseConnection.getConnection()) {
            // Check if pc_configurations table exists
            boolean tableExists = false;
            try (ResultSet tables = conn.getMetaData().getTables(null, null, "pc_configurations", null)) {
                if (tables.next()) {
                    tableExists = true;
                    System.out.println("✓ pc_configurations table exists");
                } else {
                    System.out.println("✗ pc_configurations table does not exist!");
                }
            }
            ...
            if (!tableExists) {
                ...
                try (PreparedStatement pstmt = conn.prepareStatement(
                        ...
                        "compatibility_check BIT(1) DEFAULT 1, " +
                        "FOREIGN KEY (order_id) REFERENCES orders(order_id)" +
                        ")")) {
                    pstmt.executeUpdate();
                    System.out.println("  ✓ pc_configurations table created successfully with correct schema");
                    
                    // Add sample data for testing if needed
                    addSamplePCConfigurations(conn);
                }
            } else {
                // Check if compatibility_check column exists
                boolean hasCompatibilityCheckColumn = false;
                try (ResultSet columns = conn.getMetaData().getColumns(null, null, "pc_configurations", "compatibility_check")) {
                    if (columns.next()) {
                        hasCompatibilityCheckColumn = true;
                        System.out.println("  ✓ compatibility_check column exists");
                    }
                }
                
                // Add compatibility_check column if it doesn't exist
                if (!hasCompatibilityCheckColumn) {
                    System.out.println("  Adding missing compatibility_check column...");
                    try {
                        PreparedStatement pstmt = conn.prepareStatement(
                            "ALTER TABLE pc_configurations ADD COLUMN compatibility_check BIT(1) DEFAULT 1");
                        pstmt.executeUpdate();
                        System.out.println("  ✓ compatibility_check column added successfully");
                    } catch (SQLException e) {
                        System.out.println("  Failed to add compatibility_check column: " + e.getMessage());
                    }
                }
                
                // Ensure component ID columns exist
                String[] componentColumns = {
                    "cpu_id", "gpu_id", "ram_id", "motherboard_id", "storage_id", 
                    "case_id", "power_supply_id", "cpu_cooler_id", "os_id"
                };
                
                for (String column : componentColumns) {
                    boolean hasColumn = false;
                    try (ResultSet columns = conn.getMetaData().getColumns(null, null, "pc_configurations", column)) {
                        if (columns.next()) {
                            hasColumn = true;
                        }
                    }
                    ...
                    if (!hasColumn) {
                        System.out.println("  Adding missing " + column + " column...");
                        try {
                            PreparedStatement pstmt = conn.prepareStatement(
                                "ALTER TABLE pc_configurations ADD COLUMN " + column + " INT NULL");
                            pstmt.executeUpdate();
                            System.out.println("  ✓ " + column + " column added successfully");
                        } catch (SQLException e) {
                            System.out.println("  Failed to add " + column + " column: " + e.getMessage());
                        }
                    }
                }
            }
        } catch (SQLException e) {
            System.err.println("Error validating pc_configurations table: " + e.getMessage());
            e.printStackTrace();
        }
    }
      private static void addSamplePCConfigurations(Connection conn) {
        System.out.println("  Adding sample PC configurations...");
        
        // First, get a list of existing orders
        try (PreparedStatement pstmt = conn.prepareStatement("SELECT order_id FROM orders LIMIT 5")) {
            ResultSet rs = pstmt.executeQuery();
            
            while (rs.next()) {
                int orderId = rs.getInt("order_id");
                
                // Add sample configuration for each order
                try (PreparedStatement insertStmt = conn.prepareStatement(
                        "INSERT INTO pc_configurations (order_id, cpu_id, gpu_id, ram_id, motherboard_id, storage_id, price, item_count, estimated_delivery_date, compatibility_check) " +
                        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
                    insertStmt.setInt(1, orderId);
                    insertStmt.setInt(2, 1);  // Sample CPU ID
                    insertStmt.setInt(3, 3);  // Sample GPU ID
                    insertStmt.setInt(4, 7);  // Sample RAM ID
                    insertStmt.setInt(5, 5);  // Sample Motherboard ID
                    insertStmt.setInt(6, 9);  // Sample Storage ID
                    insertStmt.setDouble(7, 999.99);  // Total price
                    insertStmt.setInt(8, 1);  // Item count
                    insertStmt.setDate(9, new java.sql.Date(System.currentTimeMillis() + 7 * 86400000L));  // 7 days
                    insertStmt.setBoolean(10, true);  // Compatible
                    
                    insertStmt.executeUpdate();
                    System.out.println("  ✓ Added PC configuration for order #" + orderId);
                } catch (SQLException e) {
                    System.out.println("  Failed to add PC configuration: " + e.getMessage());
                    
                    // Try alternative format if first attempt fails
                    try (PreparedStatement altStmt = conn.prepareStatement(
                            "INSERT INTO pc_configurations (order_id, price, item_count, estimated_delivery_date, compatibility) " +
                            "VALUES (?, ?, ?, ?, ?)")) {
                        altStmt.setInt(1, orderId);
                        altStmt.setDouble(2, 999.99);
                        altStmt.setInt(3, 1);
                        altStmt.setDate(4, new java.sql.Date(System.currentTimeMillis() + 7 * 86400000L));
                        altStmt.setString(5, "Compatible");
                        
                        altStmt.executeUpdate();
                        System.out.println("  ✓ Added PC configuration (alt format) for order #" + orderId);
                    } catch (SQLException e2) {
                        System.out.println("  Failed to add PC configuration (alt format): " + e2.getMessage());
                    }
                }
            }
        } catch (SQLException e) {
            System.err.println("Error adding sample PC configurations: " + e.getMessage());
        }
    }
